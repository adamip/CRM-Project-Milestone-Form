...
ALTER VIEW dbo.vw_ProjectMilestoneDates_v2
AS 
SELECT DISTINCT Pe.new_Title AS [Project Title]
	/* , Pe.new_ProjectID
	, Me01.new_Project
	, Me01.new_Project 
	, Me01.cust_Milestone */

	, ISNULL( Ow1.Name, '') AS [Owner/PM]
	, ISNULL( Op1.[Value], '') AS [Partner]
	, ISNULL( Op2.[Value], '') AS [Project Type Detail]
	, Pe.cust_ServicesRevenue AS [Project Services Revenue]
	, ISNULL( Pe.new_SalesOrder_temp, '')  AS [Sales Order]
	, ISNULL( Ow2.Name, '')  AS [Sales Person]
	, Op3.[Value] AS [Region]
	...
	LEFT JOIN new_projectBase AS P
		ON P.new_ProjectID = Pe.new_ProjectID 	
	LEFT JOIN OwnerBase AS Ow1 
		ON Ow1.OwnerID = P.OwnerID
	LEFT JOIN OwnerBase AS Ow2 
		ON Ow2.OwnerID = Pe.new_SalesPerson 
	LEFT JOIN StringMapBase AS Op1 
		ON Op1.AttributeName LIKE 'cust_partner' AND Op1.AttributeValue = Pe.cust_Partner AND Op1.ObjectTypeCode = 10002	
	LEFT JOIN StringMapBase AS Op2 
		ON Op2.AttributeName LIKE 'cust_ProductInterest' AND Op2.AttributeValue = Pe.cust_ProductInterest	
	LEFT JOIN StringMapBase AS Op3 
		ON Op3.AttributeName LIKE 'cust_Region' AND Op3.AttributeValue = Pe.cust_Region
		
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me01 
		ON Pe.new_ProjectID = Me01.new_Project AND Me01.cust_Milestone = 832090001
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me02 
		ON Pe.new_ProjectID = Me02.new_Project AND Me02.cust_Milestone = 832090002
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me03 
		ON Pe.new_ProjectID = Me03.new_Project AND Me03.cust_Milestone = 832090003
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me04 
		ON Pe.new_ProjectID = Me04.new_Project AND Me04.cust_Milestone = 832090004
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me05 
		ON Pe.new_ProjectID = Me05.new_Project AND Me05.cust_Milestone = 832090005
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me06 
		ON Pe.new_ProjectID = Me06.new_Project AND Me06.cust_Milestone = 832090006
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me07 
		ON Pe.new_ProjectID = Me07.new_Project AND Me07.cust_Milestone = 832090007
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me08 
		ON Pe.new_ProjectID = Me08.new_Project AND Me08.cust_Milestone = 832090008
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me09 
		ON Pe.new_ProjectID = Me09.new_Project AND Me09.cust_Milestone = 832090009
	LEFT JOIN cust_projectmilestoneSExtensionBase AS Me10 
		ON Pe.new_ProjectID = Me10.new_Project AND Me10.cust_Milestone = 832090010
		
	WHERE Pe.cust_Region = 279640000 /* selected Americas only */
		AND Pe.new_Title NOT LIKE '%Molecular Devices%'
		AND Pe.new_Title NOT LIKE '%MolDev%'
		AND Pe.cust_ServicesRevenue IS NOT NULL 
		AND Pe.cust_ServicesRevenue <> 0;