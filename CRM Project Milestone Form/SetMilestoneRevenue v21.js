/*
	...
	Adam Ip																	2014-05-22
		function MilestoneSavedDate()

	Adam Ip																	2014-08-18		
		function Milestone_OnSave( executionObj )
		function checkMandatoryDates()		
*/
...
function MilestoneSavedDate()
{
	try
	{
		var dActual = myGetValue( "cust_actualrevenuedate" );
		var dPlanned = myGetValue( "cust_plannedrevenuedate" );

		if ( DebugModeProjectMilestone )
			window.alert( "function MilestoneSavedDate\nActual Revenue Date\t" + dActual + "\nPlanned Revenue Date\t" + dPlanned );	
		
		mySetValue( "cust_actualrevenuedateprevious", dActual );
		mySetValue( "cust_plannedrevenuedateprevious", dPlanned );	
	}		
	catch( err )
	{
		window.alert( "function MilestoneSavedDate error code " + err );
	}
}
		
/****************************************************************************************/
function checkMandatoryDates( executionObj )
{
	try
	{
		if( DebugModeProjectMilestone )
			window.alert( "function checkMandatoryDates" );	

		if( myGetValue( "cust_revenue" ) != null && myGetValue( "cust_revenue" ) != 0 )
			{
			if( myGetValue( "cust_plannedrevenuedate" ) == null )
				{
				mySetFocus( "cust_plannedrevenuedate" );
				window.alert( "\'Planned Revenue Date\' is a mandatory field.\n\nPlease select a value." );
				return true;
				}
			else if( myGetValue( "cust_actualrevenuedate" ) == null )
				{
				mySetFocus( "cust_actualrevenuedate" );
				window.alert( "\'Actual Revenue Date\' is a mandatory field.\n\nPlease select a value." );
				return true;
				}
			}
		if( myGetValue( "new_milestonetargetstartdate" ) == null )
			{
			mySetFocus( "new_milestonetargetstartdate" );
			window.alert( "\'Target Start Date\' is a mandatory field.\n\nPlease select a value." );
			return true;
			}
		else if( myGetValue( "new_milestoneactualstartdate" ) == null )
			{
			mySetFocus( "new_milestoneactualstartdate" );
			window.alert( "\'Actual Start Date\' is a mandatory field.\n\nPlease select a value." );
			return true;
			}
		else if( myGetValue( "cust_milestonetargetdate" ) == null )
			{
			mySetFocus( "cust_milestonetargetdate" );
			window.alert( "\'Target Completion Date\' is a mandatory field.\n\nPlease select a value." );
			return true;
			}
		else if( myGetValue( "cust_milestonecompletiondate" ) == null )
			{
			mySetFocus( "cust_milestonecompletiondate" );
			window.alert( "\'Actual Completion Date\' is a mandatory field.\n\nPlease select a value." );
			return true;
			}					
		return false;			
	}		
	...
	{
		window.alert( "function checkMandatoryDates error code " + err );
	}
}
...
function Milestone_OnSave( executionObj )
{
	...
	{
		if ( DebugModeProjectMilestone )
			window.alert( "function Milestone_OnSave" );	

			if( myGetValue( "new_completedyn" ) == true )
				{
				if( checkMandatoryDates() == true )
					StopFormSaveEvent( executionObj );
				}	
	}		
	catch( err )
	{
		window.alert( "function Milestone_OnSave error code " + err );
	}
}
