...
function EnableCompletionDateField()
{
	...
	{
		if ( DebugModeProjectMilestone )
			window.alert( "function EnableCompletionDateField\nnew_completedyn is disabled ? " + myGetDisabled( "new_completedyn" ) + "\n\ncust_milestonecompletiondate is disabled ? " + myGetDisabled( "cust_milestonecompletiondate" ));		

		if( myGetDisabled( "new_completedyn" )	== true || myGetDisabled( "cust_milestonecompletiondate" )	== true )
		{
			...
			/* 	D48B4A4F-9F94-E111-AFED-00155D031C01 is Kent Pylant 
				...
				0DAF7346-9D94-E111-AFED-00155D031C01 is Adam Ip */
			var sUsers = new Array ( "{D48B4A4F-9F94-E111-AFED-00155D031C01}", 
									 "{14C4CD0E-DC38-E211-96F5-00155D031C01}",	
									 "{0DAF7346-9D94-E111-AFED-00155D031C01}" );	
			var sUserID = Xrm.Page.context.getUserId();			
			for( i = 0, proc = false; i < sUsers.length; i++ )
				if( sUsers[i].toUpperCase() == sUserID.toUpperCase())
					proc = true;
			if ( DebugModeProjectMilestone )
				window.alert( "function EnableCompletionDateField\nproc = " + proc + "\nsUserID = " + sUserID );		
			if( proc == true )
